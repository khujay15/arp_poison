        ...
        if(ntohs(arp->oper) == ARP_REPLY)
         {
              ...
              else if(arp->tip[3]==attacker_ip[3] && arp->tip[2]==attacker_ip[2] && isgateway==true)
              {
...
//            memmove(temp,packet,header->len-1);
        ...
        else if(same_gateway==true )
        {
            ...
            for(int i=6;i<12;i++)
            Tovictim[i] = attacker_mac[i];

            cout<<Tovictim<<endl;
            u_char* temp = (u_char*)packet;

           // memmove(temp,packet,header->len);
